...
foreach ($w in $found) {
    ...
    [Win32]::ShowWindow($w.Handle, 9) | Out-Null   # SW_RESTORE
    [Win32]::SetForegroundWindow($w.Handle) | Out-Null
    Start-Sleep -Milliseconds 300
    $bmp = New-Object System.Drawing.Bitmap($width, $height)
    $g = [System.Drawing.Graphics]::FromImage($bmp)
    $hdc = $g.GetHdc()
    $ok = [Win32]::PrintWindow($w.Handle, $hdc, 0)   # PW_CLIENTONLY? no: 0 = full window
    if (-not $ok) {
        $ok = [Win32]::PrintWindow($w.Handle, $hdc, 2) # PW_RENDERFULLCONTENT
    }
    $g.ReleaseHdc($hdc)
    $g.Dispose()
    $path = "C:\rocket-autopilot-game\docs\win_$i.png"
